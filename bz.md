...
```cpp
如果要在结构体中使用lower_bound和upper_bound，把我们需要查找的数封装成一个结构体。然后才可以在结构体重进行查找。即使我们只需要针对某一维进行查找，也需要把整个结构体构造出来。
如下：
1.重载
struct Node {
    int a, b;
    bool operator<(const Node& o) const {
        return a < o.a;  // 按 a 排序/查找
    }
};
vector<Node> v = {{1,2}, {3,4}, {5,6}};
sort(v.begin(), v.end());  // 用 operator< 排序
auto it = lower_bound(v.begin(), v.end(), Node{4, 0}); 
2.cmp
struct Node {
    int a, b;
};
bool cmpA(const Node& x, const Node& y) { return x.a < y.a; }
vector<Node> v = {{5,1}, {3,4}, {1,9}};
sort(v.begin(), v.end(), cmpA);  // 按 a 排: {1,9}, {3,4}, {5,1}
auto it = lower_bound(v.begin(), v.end(), Node{4,0}, cmpA);  // 找 a>=4
```
...
void  creat(int l,int r,int i){
	if(l==r){
		...
		int mid=l+(r-l)/2;//严格从1~n，每次从中点mid划分
		creat(l,mid,i << 1);//i*2
		creat(mid+1,r,i<<1 | 1);//i*2+1
		up(i);
	}
	ad[i]=0;
}
...
//查询函数
int  check(int nowl,int nowr,int l,int r,int i){//每次任务中nowl，nowr固定不变，在变化的l和r区间中寻找
	if(nowl <= l && r<=nowr){
		return sum[i];//如果目标区间小于任务区间，直接选择
	}
	int mid=l+(r-l)/2;
	down(i,mid-l+1,r-mid);
	int ans=0;
	if(nowl <= mid ){
		ans+=check(nowl,nowr,l,mid,i<<1);
	}
	if(nowr >mid){
		ans+=check(nowl,nowr,mid+1,r,i<<1 | 1);
	}
	return ans;
}

void add(int nowl,int nowr,int v,int l,int r,int i){
	if(nowl <= l && r<=nowr){//全包则不下发，直接修改大的，其余进入懒数列待处理
		lazy(i,v,r-l+1);
	}
	else{
		int mid=l+(r-l)/2;
		down(i,mid-l+1,r-mid);//优先处理待处理的懒数列
		if(nowl <=mid){//不全包则下发
			add(nowl,nowr,v,l,mid,i<<1);
		}
		if(nowr >mid){
			add(nowl,nowr,v,mid+1,r,i<<1 | 1);
		}
		up(i);
	}
}
...
//下发懒数列
void down(int i,int ln,int rn){//ln,rn指l~mid的长度
	if(ad[i]!=0){
		lazy(i<<1 ,ad[i],ln);
		lazy(i<<1 |1 ,ad[i],rn);
		ad[i]=0;
